...
# ===== 自定义修改：默认开启 vless+ws+argo 模式 =====
# 如果用户运行脚本时没有传入任何协议变量（vlpt/vmpt/vwpt/hypt/tupt/xhpt/vxpt/anpt/sspt/arpt/sopt/nvpt/xupt/xcpt），
# 且没有手动指定 argo 变量，则自动默认启用：Vless-ws + Argo隧道（临时隧道，除非同时提供 agn/agk 则为固定隧道）
if [ -z "${vlpt+x}" ] && [ -z "${vmpt+x}" ] && [ -z "${vwpt+x}" ] && [ -z "${hypt+x}" ] && [ -z "${tupt+x}" ] && [ -z "${xhpt+x}" ] && [ -z "${vxpt+x}" ] && [ -z "${anpt+x}" ] && [ -z "${sspt+x}" ] && [ -z "${arpt+x}" ] && [ -z "${sopt+x}" ] && [ -z "${nvpt+x}" ] && [ -z "${xupt+x}" ] && [ -z "${xcpt+x}" ]; then
export vwpt="${vwpt:-}"
...
echo "可自行更换优选IP域名，如是回源端口需手动修改443或者80系端口"
vl_vw_cdn_link="vless://$uuid@$cdnip1:$port_vw?encryption=none&type=ws&host=$xvvmcdnym&path=$uuid-vw#${sxname}vl-ws-cdn-$hostname"
...
echo "$vwatls_link2" >> "$HOME/agsbx/jhsub.txt"
vwatls_link3="vless://$uuid@yg3.ygkkk.dpdns.org:2053?encryption=none&type=ws&host=$argodomain&path=$uuid-vw&security=tls&sni=$argodomain&fp=chrome&insecure=0&allowInsecure=0#${sxname}vless-ws-tls-argo-$hostname-2053"
echo "$vwatls_link3" >> "$HOME/agsbx/jhsub.txt"
vwatls_link4="vless://$uuid@yg4.ygkkk.dpdns.org:2083?encryption=none&type=ws&host=$argodomain&path=$uuid-vw&security=tls&sni=$argodomain&fp=chrome&insecure=0&allowInsecure=0#${sxname}vless-ws-tls-argo-$hostname-2083"
echo "$vwatls_link4" >> "$HOME/agsbx/jhsub.txt"
vwatls_link5="vless://$uuid@yg5.ygkkk.dpdns.org:2087?encryption=none&type=ws&host=$argodomain&path=$uuid-vw&security=tls&sni=$argodomain&fp=chrome&insecure=0&allowInsecure=0#${sxname}vless-ws-tls-argo-$hostname-2087"
echo "$vwatls_link5" >> "$HOME/agsbx/jhsub.txt"
vwatls_link6="vless://$uuid@[2606:4700::0]:2096?encryption=none&type=ws&host=$argodomain&path=$uuid-vw&security=tls&sni=$argodomain&fp=chrome&insecure=0&allowInsecure=0#${sxname}vless-ws-tls-argo-$hostname-2096"
echo "$vwatls_link6" >> "$HOME/agsbx/jhsub.txt"
vwa_link7="vless://$uuid@$cdnip2:80?encryption=none&type=ws&host=$argodomain&path=$uuid-vw&security=none#${sxname}vless-ws-argo-$hostname-80"
echo "$vwa_link7" >> "$HOME/agsbx/jhsub.txt"
vwa_link8="vless://$uuid@yg7.ygkkk.dpdns.org:8080?encryption=none&type=ws&host=$argodomain&path=$uuid-vw&security=none#${sxname}vless-ws-argo-$hostname-8080"
echo "$vwa_link8" >> "$HOME/agsbx/jhsub.txt"
vwa_link9="vless://$uuid@yg8.ygkkk.dpdns.org:8880?encryption=none&type=ws&host=$argodomain&path=$uuid-vw&security=none#${sxname}vless-ws-argo-$hostname-8880"
echo "$vwa_link9" >> "$HOME/agsbx/jhsub.txt"
vwa_link10="vless://$uuid@yg9.ygkkk.dpdns.org:2052?encryption=none&type=ws&host=$argodomain&path=$uuid-vw&security=none#${sxname}vless-ws-argo-$hostname-2052"
echo "$vwa_link10" >> "$HOME/agsbx/jhsub.txt"
vwa_link11="vless://$uuid@yg10.ygkkk.dpdns.org:2082?encryption=none&type=ws&host=$argodomain&path=$uuid-vw&security=none#${sxname}vless-ws-argo-$hostname-2082"
echo "$vwa_link11" >> "$HOME/agsbx/jhsub.txt"
vwa_link12="vless://$uuid@yg11.ygkkk.dpdns.org:2086?encryption=none&type=ws&host=$argodomain&path=$uuid-vw&security=none#${sxname}vless-ws-argo-$hostname-2086"
echo "$vwa_link12" >> "$HOME/agsbx/jhsub.txt"
